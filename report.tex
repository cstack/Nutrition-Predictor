...
We took the best performing algorithms (Gradient Boosting Regression, Support Vector Regression, K-Nearest Neighbors, and our custom pipeline combining K-Nearest Neighbors and K-Means) and continued by training them on larger datasets with 5 fold cross-validation over the hyperparamters. We also ran these algorithms several times and averaged the results to get a more reliable idea of their performance. It is also interesting to note that “normal” Linear Regression produced an error so high that it did not even fit on the bar graph shown above.

Although Gaussian Process regression performed well in our initial small-dataset experiments, the algorithm proved too slow to be feasible when we moved up to 3,000 or 10,000 examples. This may be because the algorithm is not suited for high dimensional space. It loses efficiency when there are more than a few dozens of features\cite{gaussian}, while our data has over 1,000 features.

The K-Nearest Neighbors algorithm used to create the chart in Fig. 1 used uniform weights for the K-nearest neighbors, as opposed to distance-based weights. We decided to pursue both flavors of KNN and compare their performance for our final results.
...
\caption{Performance of various regression algorithms on Calorie data.}
\label{Figure 3}
\end{figure}
...
\caption{Comparison of error for Calories vs. CPG}
\label{Figure 4}
\end{figure}
...
\bibitem{NLTK} “Natural Language Toolkit.” NLTK 3.0 documentation. N.p., n.d. Web. 20 Dec. 2013. \texttt{http://nltk.org/}

\bibitem{gaussian} "Gaussian Processes." Scikit-learn 0.14 Documentation. N.p., n.d. Web. 20 Dec. 2013. \texttt{http://scikit-learn.org/stable/modules/gaussian\_process.html}

\bibitem{svm} "Support Vector Machines." Scikit-learn 0.14 Documentation. N.p., n.d. Web. 20 Dec. 2013. \texttt{http://scikit-learn.org/stable/modules/svm.html\#regression}
